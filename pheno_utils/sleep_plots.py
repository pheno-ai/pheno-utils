...
# %% auto 0
__all__ = ['CHANNELS', 'DEFAULT_CHANNELS', 'COLOR_GROUPS', 'ENUMS', 'CHANNEL_LIMS', 'plot_sleep', 'plot_events', 'plot_raw',
...
def plot_sleep(events: pd.DataFrame, raw: pd.DataFrame,
    ...
    """
    Plot sleep events and raw data.

    Args:

        events (pd.DataFrame): A pandas dataframe containing sleep events data.
        raw (pd.DataFrame): A pandas dataframe containing raw data.
        ...
        **kwargs: Additional arguments to be passed to plot_raw().

    Returns:

        None
    """
    nC = min([len(channel_filter), raw.index.get_level_values('source').nunique()])
    if xlim is not None:
        trim_to_events = True
    ...
    with sns.axes_style(style):
        try:
            plot_raw(raw, array_index=array_index, y_filter=channel_filter, ax=ax[:-1],
                     **kwargs)
            ...
            print(f'plot_raw failed due to:\n{err}')
            fig.clf()
...
def plot_raw(raw_channels: pd.DataFrame, array_index: Optional[int]=None,
    ...
    """ plot raw data for a given participant and array_index """
    # set colors
    ...
    # filter data
    if (array_index is not None) and (('array_index' in raw_channels.columns) or ('array_index' in raw_channels.index.names)):
        data = raw_channels.query('array_index == @array_index').copy()
    else:
        data = raw_channels.copy()
    # extract time and channel name
    if time_col in raw_channels.index.names:
    ...
    for i, (source, d) in enumerate(data.groupby('source', sort=False)):
        if (source not in CHANNELS) or (y_filter is not None and source not in y_filter):
            print(f'plot_raw: skipping {source}')
            ax_shift += 1
            continue
        iax = i - ax_shift
